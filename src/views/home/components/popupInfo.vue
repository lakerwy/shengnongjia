  ...
  <div class="container boxbg">
    <div class="border">
      ...
      <div v-if="data.infoType === 'warn'" class="warn-info">
        ...
        <div class="content">
          <span class="label">时间：</span>{{ data.begin_time | dateFormate1 }}
        </div>
      </div>
      <div
        v-if="data.infoType === 'water' || data.infoType === 'gas'"
        class="warn-info"
      >
      ...
      <div
        v-if="localData && localData.infoType === 'fireTown'"
        class="camera-info"
      >
        <div class="video">
          <player
        ...
        <div class="info noWrap">
          ...
          <div class="content">
            <span class="label">单位：</span>{{ data.unitName }}
          </div>
          <div class="content">
            <span class="label">经度：</span>{{ data.longitude }}
          </div>
          <div class="content">
            <span class="label">纬度：</span>{{ data.latitude }}
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  watch: {
    data() {
      console.log(this.data.infoType);
      this.init();
    }
  },
  methods: {
    init() {
      if (this.data) {
        ...
        } else if (this.data.infoType === "fireTown") {
          this.getFireInfo();
        }
      }
    },
    ...
    getFireInfo() {
      this.localData = this.data;
      if (this.data && Array.isArray(this.data.relationDevices)) {
        this.data.relationDevices.forEach(async x => {
          if (x.codeType === "kjg") {
            let params = {
              stream: x.liveSerial,
              videoType: 2
